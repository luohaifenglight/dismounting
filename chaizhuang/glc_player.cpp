...
, m_ClipBoard(0, NULL)
{
	...
	m_pDisassemblymanagerview = new Disassemblymanagerview(&m_OpenglView, &m_FileEntryHash, albumManagementWindow);
	m_pModelManagerView = new ModelManagerView(&m_OpenglView, action_Property, actionHide_unselected, actionCopy, actionCut, actionPaste, &m_ClipBoard, albumManagementWindow);
	connect(m_pModelManagerView, SIGNAL(currentModelProperties()), m_pAlbumManagerView, SLOT(modelProperties()));
	// LeftSideDock
	m_pLeftSideDock = new LeftSideDock(m_pAlbumManagerView, m_pDisassemblymanagerview,m_pModelManagerView, &m_FileEntryHash, albumManagementWindow);
	...
	connect(m_pDisassemblymanagerview, SIGNAL(showAll()), this, SLOT(showAll()));
...
void glc_player::loadProjects()
{
	...
	QDir thisPath(moduleDir);
	QStringList tempStrList;
	tempStrList = thisPath.entryList(QDir::Filter::NoFilter, QDir::DirsFirst);
	tempStrList.removeAt(1);//É¾³ý¡°..¡±  
	tempStrList.removeAt(0);//É¾³ý¡°.¡±  

	QString tempStr;
	for (int i = 0; i < tempStrList.count(); i++)
	{
		tempStr = moduleDir + "/" + tempStrList.at(i);
		QStringList fileNames = fileNameFromGivenPath(tempStr, filters);
		qDebug() << fileNames.count() << " File(s) found";
		if (!fileNames.isEmpty())
		{
			addItems(fileNames);
