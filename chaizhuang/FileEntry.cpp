...
void FileEntry::duplicateWorld(QString srcWorld, QString dstWorld)
{
	if (srcWorld.isEmpty())
	{
		if (m_modelEntryHash.contains(dstWorld))
		{
			GLC_World *oldWord = m_modelEntryHash[dstWorld];
			delete oldWord;
		}
		m_modelEntryHash[dstWorld] = new GLC_World(m_World);
	}
}

bool FileEntry::ExtractOccurence(QString srcWorld, QString OccurenceName)
{
	if (m_modelEntryHash.contains(srcWorld))
	{
		GLC_World *world = m_modelEntryHash[srcWorld];
		foreach(GLC_StructOccurence* pOccurence, (world->rootOccurence()->children()))
		{
			if (pOccurence->structInstance()->name() == OccurenceName)
			{
				pOccurence->makeOrphan();
				if (!m_modelEntryHash.contains(OccurenceName))
				{
					GLC_World *world = new GLC_World();
					world->rootOccurence()->addChild(pOccurence);
					m_modelEntryHash[OccurenceName] = world;
					return true;
				}
			}
		}
...
void FileEntry::setWorld(QString worldName)
{
	...
	{
		if (m_modelEntryHash.contains(worldName))
		{
			GLC_World *World = m_modelEntryHash[worldName];
			setWorld(*World);
		}
	}		
}
